...
void Install() {
    ...
    stl::WriteThunkCall<AddSkillExperience>(REL::Relocation {RELOCATION_ID(39413, 40488), 0x25});
...
// The hook signature must match the engine call.
...
void AddSkillExperience::thunk(
    ...
    if (Physical::ShouldSuppressSkillXP(a_av)) {
        SKSE::log::debug("Ignored XP from blocking | skill={} | xp={:.2f}", RE::ActorValueToString(a_av), a_xp);
        return;
    }
